...
public class PhoneNumberDAO {
	...
	public List<PhoneNumberModel> getNumbersList(int id) {
		...
		return jdbc
				.query("SELECT UCASE(t.operator) AS operator, t.koniec_umowy, t.numer, REPLACE(t.typ_uslugi, '_', ' ') AS typ_uslugi, t.dlugosc_umowy, k.nazwa_firmy, CONCAT(k.imie, ' ', k.nazwisko, '   NIP:', k.nip) AS dane_klienta FROM telefon t JOIN klient k ON t.klient_id=k.id WHERE k.pracownik_id="
	...
	public boolean updateNumber(PhoneNumberModel number) {
		BeanPropertySqlParameterSource params = new BeanPropertySqlParameterSource(
				number);
		return jdbc
				.update("UPDATE telefon SET numer=:numer, operator=:operator, dlugosc_umowy=:dlugoscUmowy, poczatek_umowy=:poczatekUmowy, koniec_umowy=:koniecUmowy, typ_uslugi=:typUslugi, notatka=:notatka, klient_id=:klientId",
						params) == 1;
	}
	...
	public boolean createNumber(PhoneNumberModel number) {
		BeanPropertySqlParameterSource params = new BeanPropertySqlParameterSource(
				number);
		return jdbc.update("INSERT INTO telefon (numer, operator, dlugosc_umowy, poczatek_umowy, koniec_umowy, typ_uslugi, notatka, klient_id) VALUES (:numer, :operator, :dlugoscUmowy, :poczatekUmowy, :koniecUmowy, :typUslugi, notatka, :klientId)",
				params) == 1;
	}
