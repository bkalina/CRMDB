...
public class ScheduleDAO {
	...
	public List<ScheduleModel> getTermsList(int id) {
		...
		return jdbc
				.query("SELECT z.id, z.data_zdarzenia, z.godzina, z.opis, z.notatka, z.pracownik_id FROM zdarzenie z where z.pracownik_id="
						+ id, new RowMapper<ScheduleModel>() {
	...
	public ScheduleModel getTerm(int termId) {
		...
		return jdbc.queryForObject("SELECT * FROM zdarzenie WHERE id=:id",
				param, new RowMapper<ScheduleModel>() {
					public ScheduleModel mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						ScheduleModel scheduleModel = new ScheduleModel();
	...
	public boolean updateTerm(ScheduleModel schedule, int termId) {
		BeanPropertySqlParameterSource params = new BeanPropertySqlParameterSource(
				schedule);
		return jdbc
				.update("UPDATE zdarzenie SET data_zdarzenia=:dataZdarzenia, godzina=:godzina, opis=:opis, notatka=:notatka, pracownik_id=:pracownikId WHERE id="
						+ termId, params) == 1;
	}
	...
	public boolean createTerm(ScheduleModel schedule) {
		BeanPropertySqlParameterSource params = new BeanPropertySqlParameterSource(
				schedule);
		return jdbc
				.update("INSERT INTO zdarzenie (data_zdarzenia, godzina, opis, notatka, pracownik_id) VALUES (:dataZdarzenia, :godzina, :opis, :notatka, :pracownikId)",
						params) == 1;
	}
